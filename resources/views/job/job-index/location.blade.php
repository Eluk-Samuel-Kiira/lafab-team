    ...
    <div class="card-header mt-6">
        <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1 me-5">
                ...
                <input type="text" id="searchInput" class="form-control form-control-solid w-250px ps-13" placeholder="Search locations..." />
            </div>
            <div>
        ...
        <div class="card-toolbar">
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_add_location">
...
<!-- Add Location Modal -->
<div class="modal fade" id="kt_modal_add_location" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="fw-bold">Add Job Location</h2>
            ...
            <div class="modal-body scroll-y mx-5 my-7">
                <form id="addLocationForm">
                    ...
                    <div class="fv-row mb-7">
                        ...
                        <input type="text" class="form-control form-control-solid" name="district" placeholder="e.g., Kampala, Nairobi, Sydney" required />
                        <div class="text-muted fs-7 mt-1">Example: Kampala, Nairobi, Sydney, Kigali</div>
                    </div>
                    
                    <div class="fv-row mb-7">
                        <label class="fw-semibold fs-6 mb-2">City</label>
                        <input type="text" class="form-control form-control-solid" name="city" placeholder="e.g., Central, Downtown" />
                        <div class="text-muted fs-7 mt-1">Specific city or area within the district (optional)</div>
                    </div>
                    
                    <div class="fv-row mb-7">
                        <label class="fw-semibold fs-6 mb-2">Description</label>
                        <textarea class="form-control form-control-solid" name="description" rows="3" placeholder="Brief description of this location (optional)"></textarea>
                    </div>
                    
                    <div class="row mb-7">
                        <div class="col-md-6">
                            <label class="fw-semibold fs-6 mb-2">Latitude</label>
                            <input type="number" step="0.00000001" class="form-control form-control-solid" name="latitude" placeholder="-1.2921" />
                        </div>
                        <div class="col-md-6">
                            <label class="fw-semibold fs-6 mb-2">Longitude</label>
                            <input type="number" step="0.00000001" class="form-control form-control-solid" name="longitude" placeholder="36.8219" />
                        </div>
                    </div>
                    ...
                    <div class="row mb-7">
                        ...
                        <div class="col-md-6">
                            <div class="form-check form-switch form-check-custom form-check-solid mt-8">
                                <input class="form-check-input" type="checkbox" name="is_capital" />
                                <label class="form-check-label fw-semibold">Capital City</label>
                            </div>
                        </div>
                    </div>
...
<!-- Edit Location Modal -->
<div class="modal fade" id="kt_modal_edit_location" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="fw-bold">Edit Job Location</h2>
                <div class="btn btn-icon btn-sm btn-active-icon-primary" data-bs-dismiss="modal">
                    <i class="ki-duotone ki-cross fs-1"><span class="path1"></span><span class="path2"></span></i>
                </div>
            </div>
            <div class="modal-body scroll-y mx-5 my-7">
                <form id="editLocationForm">
                    ...
                    <div class="fv-row mb-7">
                        <label class="required fw-semibold fs-6 mb-2">District</label>
                        <input type="text" class="form-control form-control-solid" name="district" id="edit_district" required />
                    </div>
                    
                    <div class="fv-row mb-7">
                        <label class="fw-semibold fs-6 mb-2">City</label>
                        <input type="text" class="form-control form-control-solid" name="city" id="edit_city" />
                    </div>
                    ...
                    <div class="row mb-7">
                        <div class="col-md-6">
                            <label class="fw-semibold fs-6 mb-2">Latitude</label>
                            <input type="number" step="0.00000001" class="form-control form-control-solid" name="latitude" id="edit_latitude" />
                        </div>
                        <div class="col-md-6">
                            <label class="fw-semibold fs-6 mb-2">Longitude</label>
                            <input type="number" step="0.00000001" class="form-control form-control-solid" name="longitude" id="edit_longitude" />
                        </div>
                    </div>
                    ...
                    <div class="row mb-7">
                        ...
                        <div class="col-md-6">
                            <div class="form-check form-switch form-check-custom form-check-solid mt-8">
                                <input class="form-check-input" type="checkbox" name="is_capital" id="edit_is_capital" />
                                <label class="form-check-label fw-semibold">Capital City</label>
                            </div>
                        </div>
                    </div>
...
function loadLocations() {
    ...
    let url = `/admin/job-locations/data?page=${currentPage}&per_page=20`;
    ...
    fetch(url)
        ...
        .catch(err => {
            spinner.classList.add('d-none');
            window.showToast('error', 'Failed to load locations');
        });
}
...
function renderLocationsTable(locations) {
    ...
    locations.forEach(location => {
        ...
        row.insertCell(0).innerHTML = `<span class="fw-bold">${location.id}</span>`;
        row.insertCell(1).innerHTML = `<div class="fw-bold">${escapeHtml(location.district)}</div>`;
        row.insertCell(2).innerHTML = location.city ? escapeHtml(location.city) : '<span class="text-muted">-</span>';
        row.insertCell(3).innerHTML = `<span class="badge badge-light-info">${location.flag} ${location.country_name}</span>`;
        row.insertCell(4).innerHTML = location.status_badge;
        row.insertCell(5).innerHTML = location.capital_badge;
        row.insertCell(6).innerHTML = location.sort_order || 0;
        row.insertCell(7).innerHTML = `
            <div class="d-flex justify-content-end gap-2">
                <button class="btn btn-sm btn-icon btn-light" onclick="toggleStatus(${location.id}, ${location.is_active})" title="${location.is_active ? 'Deactivate' : 'Activate'}">
                    <i class="ki-duotone ki-${location.is_active ? 'disconnect' : 'check'} fs-3">
                        <span class="path1"></span>
                        <span class="path2"></span>
                    </i>
                </button>
                <button class="btn btn-sm btn-icon btn-light" onclick="editLocation(${location.id})" title="Edit">
                ...
                <button class="btn btn-sm btn-icon btn-light" onclick="deleteLocation(${location.id}, '${escapeHtml(location.district)}')" title="Delete">
...
window.changePage = function(page) {
    if (page !== currentPage && page > 0) { currentPage = page; loadLocations(); }
};
...
window.toggleStatus = function(id, current) {
    ...
    if (confirm(`Are you sure you want to ${action} this location?`)) {
        fetch(`/admin/job-locations/${id}/toggle-status`, {
            method: 'POST',
...
window.editLocation = function(id) {
    fetch(`/admin/job-locations/${id}`)
        ...
        .then(data => {
            ...
            document.getElementById('edit_is_capital').checked = data.is_capital;
            new bootstrap.Modal(document.getElementById('kt_modal_edit_location')).show();
        })
        .catch(err => window.showToast('error', 'Failed to load location details'));
};
...
window.deleteLocation = function(id, name) {
    if (confirm(`Are you sure you want to delete location "${name}"? This action cannot be undone.`)) {
        ...
        .catch(err => window.showToast('error', 'Failed to delete location'));
    }
};
...
document.getElementById('addLocationForm')?.addEventListener('submit', function(e) {
    ...
    const isActiveCheckbox = document.querySelector('#addLocationForm input[name="is_active"]');
    if (isActiveCheckbox) {
        formData.set('is_active', isActiveCheckbox.checked ? '1' : '0');
    }
    
    const isCapitalCheckbox = document.querySelector('#addLocationForm input[name="is_capital"]');
    if (isCapitalCheckbox) {
        formData.set('is_capital', isCapitalCheckbox.checked ? '1' : '0');
    }
    
    fetch('/admin/job-locations', {
        method: 'POST',
        headers: { 
    ...
    .then(data => {
        if (data.success) {
            window.showToast('success', data.message);
            const modal = bootstrap.Modal.getInstance(document.getElementById('kt_modal_add_location'));
            if (modal) modal.hide();
            this.reset();
            loadLocations();
        } else {
            if (data.errors) {
                ...
                window.showToast('error', data.message || 'Failed to create location');
            }
        }
    })
    .catch(err => {
        console.error('Error:', err);
        window.showToast('error', 'Failed to create location: ' + err.message);
    })
...
document.getElementById('editLocationForm')?.addEventListener('submit', function(e) {
    ...
    const isActiveCheckbox = document.querySelector('#editLocationForm input[name="is_active"]');
    if (isActiveCheckbox) {
        formData.set('is_active', isActiveCheckbox.checked ? '1' : '0');
    }
    
    const isCapitalCheckbox = document.querySelector('#editLocationForm input[name="is_capital"]');
    if (isCapitalCheckbox) {
        formData.set('is_capital', isCapitalCheckbox.checked ? '1' : '0');
    }
    ...
    .then(data => {
        if (data.success) {
            window.showToast('success', data.message);
            const modal = bootstrap.Modal.getInstance(document.getElementById('kt_modal_edit_location'));
            if (modal) modal.hide();
            loadLocations();
        } else {
    ...
    .catch(err => {
        console.error('Error:', err);
        window.showToast('error', 'Failed to update location: ' + err.message);
    })
    .finally(() => window.hideButtonSpinner(btn));
});
