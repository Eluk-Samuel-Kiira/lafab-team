...
function initRichEditors() {
    const editors = [
        { id: 'edit_description_editor', container: 'edit_description_editor_container', hidden: 'edit_description_hidden', placeholder: 'Enter job description...' },
        { id: 'edit_responsibilities_editor', container: 'edit_responsibilities_editor_container', hidden: 'edit_responsibilities_hidden', placeholder: 'Enter responsibilities...' },
        { id: 'edit_skills_editor', container: 'edit_skills_editor_container', hidden: 'edit_skills_hidden', placeholder: 'Enter skills...' },
        { id: 'edit_qualifications_editor', container: 'edit_qualifications_editor_container', hidden: 'edit_qualifications_hidden', placeholder: 'Enter qualifications...' },
        { id: 'edit_application_editor', container: 'edit_application_editor_container', hidden: 'edit_application_hidden', placeholder: 'Enter application procedure...' }
    ];
    
    editors.forEach(editor => {
        const container = document.getElementById(editor.container);
        const existingEditor = document.getElementById(editor.id);
        
        if (container && !existingEditor) {
            container.innerHTML = buildRichEditor(editor.id, editor.id, editor.placeholder, 150);
        }
    });
}

function buildRichEditor(id, name, placeholder, height) {
    ...
    <div class="rich-editor-wrapper border rounded overflow-hidden" data-editor-id="${id}">
        <div class="rich-editor-toolbar d-flex flex-wrap align-items-center gap-1 px-2 py-1 border-bottom bg-light">
            ...
            <button type="button" class="re-btn" id="${id}-bold" onclick="reFmt('${id}','bold')" title="Bold">
                <svg viewBox="0 0 24 24" ${s}><path d="M6 4h8a4 4 0 0 1 0 8H6z"/><path d="M6 12h9a4 4 0 0 1 0 8H6z"/></svg>
            </button>
            <button type="button" class="re-btn" id="${id}-italic" onclick="reFmt('${id}','italic')" title="Italic">
                <svg viewBox="0 0 24 24" ${s}><line x1="19" y1="4" x2="10" y2="4"/><line x1="14" y1="20" x2="5" y2="20"/><line x1="15" y1="4" x2="9" y2="20"/></svg>
            </button>
            <button type="button" class="re-btn" id="${id}-underline" onclick="reFmt('${id}','underline')" title="Underline">
                <svg viewBox="0 0 24 24" ${s}><path d="M6 3v7a6 6 0 0 0 12 0V3"/><line x1="4" y1="21" x2="20" y2="21"/></svg>
            </button>
            <div class="re-sep"></div>
            <button type="button" class="re-btn re-btn-text" onclick="reFmt('${id}','formatBlock','h2')">H2</button>
            <button type="button" class="re-btn re-btn-text" onclick="reFmt('${id}','formatBlock','h3')">H3</button>
            <button type="button" class="re-btn re-btn-text" onclick="reFmt('${id}','formatBlock','p')">P</button>
            <div class="re-sep"></div>
            ...
            <button type="button" class="re-btn re-btn-danger" onclick="richEditorClear('${id}')" title="Clear">
