...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item text-muted">Industries</li>
@endsection
...
<div class="card card-flush">
    <div class="card-header mt-6">
        <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1 me-5">
                ...
                <input type="text" id="searchInput" class="form-control form-control-solid w-250px ps-13" placeholder="Search industries..." />
            </div>
        </div>
        @can('create job industries')
        <div class="card-toolbar">
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_add_industry">
    ...
    <div class="card-body pt-0">
        <div id="loadingSpinner" class="text-center py-10 d-none">
            ...
            <p class="mt-3 text-muted">Loading industries...</p>
        </div>
                ...
                <table class="table align-middle table-row-dashed fs-6 gy-5 mb-0">
                    <thead>
                        <tr class="text-start text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                            ...
                            <th class="min-w-120px">Est. Salary</th>
...
<!-- Add Industry Modal -->
<div class="modal fade" id="kt_modal_add_industry" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="fw-bold">Add Industry</h2>
            ...
            <div class="modal-body scroll-y mx-5 my-7">
                <form id="addIndustryForm">
                    ...
                    <div class="fv-row mb-7">
                        <label class="required fw-semibold fs-6 mb-2">Industry Name</label>
                    ...
                    <div class="row mb-7">
                        <div class="col-md-6">
                            ...
                            <select class="form-select form-select-solid" name="icon" id="add_icon">
                                ...
                                <option value="ki-factory">🏭 Factory</option>
                                <option value="ki-truck">🚚 Truck</option>
                                <option value="ki-hotel">🏨 Hotel</option>
                                <option value="ki-restaurant">🍽️ Restaurant</option>
                                <option value="ki-shopping">🛍️ Shopping</option>
                                <option value="ki-bank">🏦 Bank</option>
                                <option value="ki-insurance">📋 Insurance</option>
                                <option value="ki-accounting">🧾 Accounting</option>
                                <option value="ki-law">⚖️ Law</option>
                                <option value="ki-marketing">📣 Marketing</option>
                                <option value="ki-media">🎬 Media</option>
                                <option value="ki-sport">⚽ Sport</option>
                            </select>
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="fv-row mb-7">
                        ...
                        <textarea class="form-control form-control-solid" name="description" rows="3" placeholder="Brief description of this industry (optional)"></textarea>
                    </div>
                    
                    <div class="fv-row mb-7">
                        <label class="fw-semibold fs-6 mb-2">Estimated Salary</label>
                        <input type="number" step="0.01" class="form-control form-control-solid" name="estimated_salary" placeholder="0.00" />
                        <div class="text-muted fs-7 mt-1">Average estimated salary for this industry (optional)</div>
                    </div>
                    ...
                    <div class="text-center pt-15">
                        ...
                        <button type="submit" class="btn btn-primary" id="addIndustryBtn">
                            <span class="indicator-label">Create Industry</span>
...
<!-- Edit Industry Modal -->
<div class="modal fade" id="kt_modal_edit_industry" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="fw-bold">Edit Industry</h2>
                <div class="btn btn-icon btn-sm btn-active-icon-primary" data-bs-dismiss="modal">
                    <i class="ki-duotone ki-cross fs-1"><span class="path1"></span><span class="path2"></span></i>
                </div>
            </div>
            <div class="modal-body scroll-y mx-5 my-7">
                <form id="editIndustryForm">
                    ...
                    <div class="fv-row mb-7">
                        <label class="fw-semibold fs-6 mb-2">Estimated Salary</label>
                        <input type="number" step="0.01" class="form-control form-control-solid" name="estimated_salary" id="edit_estimated_salary" placeholder="0.00" />
                    </div>
                    ...
                    <div class="text-center pt-15">
                        ...
                        <button type="submit" class="btn btn-primary" id="editIndustryBtn">
                            <span class="indicator-label">Update Industry</span>
                            <span class="indicator-progress">Please wait... <span class="spinner-border spinner-border-sm align-middle ms-2"></span></span>
                        </button>
                    </div>
                </form>
...
function loadIndustries() {
    ...
    let url = `/admin/industries/data?page=${currentPage}&per_page=20`;
    ...
    fetch(url)
        ...
        .catch(err => {
            spinner.classList.add('d-none');
            window.showToast('error', 'Failed to load industries');
        });
}
...
function renderIndustriesTable(industries) {
    ...
    industries.forEach(industry => {
        ...
        row.insertCell(0).innerHTML = `<span class="fw-bold">${industry.id}</span>`;
        row.insertCell(1).innerHTML = industry.icon ? `<span class="fs-2"><i class="${industry.icon}"></i></span>` : '<span class="text-muted">-</span>';
        row.insertCell(2).innerHTML = `<div class="fw-bold">${escapeHtml(industry.name)}</div>`;
        row.insertCell(3).innerHTML = industry.description ? escapeHtml(industry.description.substring(0, 100)) : '<span class="text-muted">-</span>';
        row.insertCell(4).innerHTML = industry.estimated_salary ? `<span class="badge badge-light-primary">$${Number(industry.estimated_salary).toLocaleString()}</span>` : '<span class="text-muted">-</span>';
        row.insertCell(5).innerHTML = industry.is_active ? '<span class="badge badge-light-success">Active</span>' : '<span class="badge badge-light-danger">Inactive</span>';
        row.insertCell(6).innerHTML = industry.sort_order || 0;
        row.insertCell(7).innerHTML = `
            <div class="d-flex justify-content-end gap-2">
                <button class="btn btn-sm btn-icon btn-light" onclick="toggleStatus(${industry.id}, ${industry.is_active})" title="${industry.is_active ? 'Deactivate' : 'Activate'}">
                    <i class="ki-duotone ki-${industry.is_active ? 'disconnect' : 'check'} fs-3">
                        <span class="path1"></span>
                        <span class="path2"></span>
                    </i>
                </button>
                <button class="btn btn-sm btn-icon btn-light" onclick="editIndustry(${industry.id})" title="Edit">
                ...
                <button class="btn btn-sm btn-icon btn-light" onclick="deleteIndustry(${industry.id}, '${escapeHtml(industry.name)}')" title="Delete">
...
window.changePage = function(page) {
    if (page !== currentPage && page > 0) { currentPage = page; loadIndustries(); }
};
...
window.toggleStatus = function(id, current) {
    ...
    if (confirm(`Are you sure you want to ${action} this industry?`)) {
        fetch(`/admin/industries/${id}/toggle-status`, {
            method: 'POST',
...
window.editIndustry = function(id) {
    fetch(`/admin/industries/${id}`)
        .then(res => res.json())
        .then(data => {
            document.getElementById('edit_industry_id').value = data.id;
            document.getElementById('edit_name').value = data.name;
            document.getElementById('edit_icon').value = data.icon || '';
            document.getElementById('edit_description').value = data.description || '';
            document.getElementById('edit_estimated_salary').value = data.estimated_salary || '';
            ...
            new bootstrap.Modal(document.getElementById('kt_modal_edit_industry')).show();
        })
        .catch(err => window.showToast('error', 'Failed to load industry details'));
};

window.deleteIndustry = function(id, name) {
    if (confirm(`Are you sure you want to delete industry "${name}"? This action cannot be undone.`)) {
        ...
        .catch(err => window.showToast('error', 'Failed to delete industry'));
    }
};

// Add Industry Form
document.getElementById('addIndustryForm')?.addEventListener('submit', function(e) {
    e.preventDefault();
    const btn = document.getElementById('addIndustryBtn');
    window.showButtonSpinner(btn);
    
    const formData = new FormData(this);
    
    const isActiveCheckbox = document.querySelector('#addIndustryForm input[name="is_active"]');
    if (isActiveCheckbox) {
        formData.set('is_active', isActiveCheckbox.checked ? '1' : '0');
    }
    
    fetch('/admin/industries', {
        method: 'POST',
        headers: { 
    ...
    .then(data => {
        if (data.success) {
            window.showToast('success', data.message);
            const modal = bootstrap.Modal.getInstance(document.getElementById('kt_modal_add_industry'));
            if (modal) modal.hide();
            this.reset();
            loadIndustries();
        } else {
            if (data.errors) {
                ...
                window.showToast('error', data.message || 'Failed to create industry');
            }
        }
    })
    .catch(err => {
        console.error('Error:', err);
        window.showToast('error', 'Failed to create industry: ' + err.message);
    })
...
document.getElementById('editIndustryForm')?.addEventListener('submit', function(e) {
    e.preventDefault();
    const btn = document.getElementById('editIndustryBtn');
    window.showButtonSpinner(btn);
    const id = document.getElementById('edit_industry_id').value;
    
    const formData = new FormData(this);
    formData.append('_method', 'PUT');
    
    const isActiveCheckbox = document.querySelector('#editIndustryForm input[name="is_active"]');
    if (isActiveCheckbox) {
        formData.set('is_active', isActiveCheckbox.checked ? '1' : '0');
    }
    ...
    .then(data => {
        if (data.success) {
            window.showToast('success', data.message);
            const modal = bootstrap.Modal.getInstance(document.getElementById('kt_modal_edit_industry'));
            if (modal) modal.hide();
            loadIndustries();
        } else {
    ...
    .catch(err => {
        console.error('Error:', err);
        window.showToast('error', 'Failed to update industry: ' + err.message);
    })
    .finally(() => window.hideButtonSpinner(btn));
});
