...
@section('page_title', 'Education Levels')
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item text-muted">Education Levels</li>
@endsection
...
<div class="card card-flush">
    <div class="card-header mt-6">
        <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1 me-5">
                ...
                <input type="text" id="searchInput" class="form-control form-control-solid w-250px ps-13" placeholder="Search education levels..." />
            </div>
            <div>
        ...
        @can('create education levels')
        <div class="card-toolbar">
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_add_education_level">
                <i class="ki-duotone ki-plus-square fs-2">
                    <span class="path1"></span><span class="path2"></span><span class="path3"></span>
                </i> Add Education Level
            </button>
        </div>
        @endcan
    </div>
    ...
    <div class="card-body pt-0">
        <div id="loadingSpinner" class="text-center py-10 d-none">
            ...
            <p class="mt-3 text-muted">Loading education levels...</p>
        </div>
        ...
        <div id="tableContainer" class="d-none">
            <div class="table-responsive">
                <table class="table align-middle table-row-dashed fs-6 gy-5 mb-0">
                    ...
                    <tbody id="educationLevelsTableBody"></tbody>
                </table>
            </div>
        ...
        <div id="noDataMessage" class="text-center py-10 d-none">
            ...
            <p class="text-muted">No education levels found.</p>
        </div>
    </div>
</div>

<!-- Add Education Level Modal -->
<div class="modal fade" id="kt_modal_add_education_level" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="fw-bold">Add Education Level</h2>
                <div class="btn btn-icon btn-sm btn-active-icon-primary" data-bs-dismiss="modal">
                    <i class="ki-duotone ki-cross fs-1"><span class="path1"></span><span class="path2"></span></i>
                </div>
            </div>
            <div class="modal-body scroll-y mx-5 my-7">
                <form id="addEducationLevelForm">
                    @csrf
                    
                    <div class="fv-row mb-7">
                        <label class="required fw-semibold fs-6 mb-2">Level Name</label>
                        <input type="text" class="form-control form-control-solid" name="name" placeholder="e.g., Bachelor's Degree, Diploma" required />
                        <div class="text-muted fs-7 mt-1">Example: Bachelor's Degree, Diploma, Master's Degree</div>
                    </div>
                    ...
                    <div class="fv-row mb-7">
                        ...
                        <textarea class="form-control form-control-solid" name="description" rows="3" placeholder="Brief description of this education level (optional)"></textarea>
                    </div>
                    ...
                    <div class="text-center pt-15">
                        ...
                        <button type="submit" class="btn btn-primary" id="addEducationLevelBtn">
                            <span class="indicator-label">Create Education Level</span>
...
<div class="modal fade" id="kt_modal_edit_education_level" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="fw-bold">Edit Education Level</h2>
                <div class="btn btn-icon btn-sm btn-active-icon-primary" data-bs-dismiss="modal">
                    <i class="ki-duotone ki-cross fs-1"><span class="path1"></span><span class="path2"></span></i>
                </div>
            </div>
            <div class="modal-body scroll-y mx-5 my-7">
                <form id="editEducationLevelForm">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="education_level_id" id="edit_education_level_id">
                    ...
                    <div class="text-center pt-15">
                        ...
                        <button type="submit" class="btn btn-primary" id="editEducationLevelBtn">
                            <span class="indicator-label">Update Education Level</span>
                            <span class="indicator-progress">Please wait... <span class="spinner-border spinner-border-sm align-middle ms-2"></span></span>
                        </button>
                    </div>
                </form>
...
function loadEducationLevels() {
    ...
    let url = `/admin/education-levels/data?page=${currentPage}&per_page=20`;
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            if (data.data.length === 0) {
                ...
                renderEducationLevelsTable(data.data);
        ...
        .catch(err => {
            spinner.classList.add('d-none');
            window.showToast('error', 'Failed to load education levels');
        });
}

function renderEducationLevelsTable(levels) {
    const tbody = document.getElementById('educationLevelsTableBody');
    tbody.innerHTML = '';
    ...
    levels.forEach(level => {
        ...
        row.insertCell(2).innerHTML = `<span class="badge badge-light-info">${level.country_flag} ${level.country_name}</span>`;
        ...
        row.insertCell(6).innerHTML = `
            <div class="d-flex justify-content-end gap-2">
                ...
                <button class="btn btn-sm btn-icon btn-light" onclick="editEducationLevel(${level.id})" title="Edit">
                ...
                <button class="btn btn-sm btn-icon btn-light" onclick="deleteEducationLevel(${level.id}, '${escapeHtml(level.name)}')" title="Delete">
...
window.changePage = function(page) {
    if (page !== currentPage && page > 0) { currentPage = page; loadEducationLevels(); }
};

window.toggleStatus = function(id, current) {
    const action = current ? 'deactivate' : 'activate';
    if (confirm(`Are you sure you want to ${action} this education level?`)) {
        fetch(`/admin/education-levels/${id}/toggle-status`, {
            method: 'POST',
...
window.editEducationLevel = function(id) {
    fetch(`/admin/education-levels/${id}`)
        .then(res => res.json())
        .then(data => {
            document.getElementById('edit_education_level_id').value = data.id;
            ...
            new bootstrap.Modal(document.getElementById('kt_modal_edit_education_level')).show();
        })
        .catch(err => window.showToast('error', 'Failed to load education level details'));
};

window.deleteEducationLevel = function(id, name) {
    if (confirm(`Are you sure you want to delete education level "${name}"? This action cannot be undone.`)) {
        fetch(`/admin/education-levels/${id}`, {
            method: 'DELETE',
            headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Content-Type': 'application/json' }
        })
        ...
        .catch(err => window.showToast('error', 'Failed to delete education level'));
    }
};

// Add Education Level Form
document.getElementById('addEducationLevelForm')?.addEventListener('submit', function(e) {
    e.preventDefault();
    const btn = document.getElementById('addEducationLevelBtn');
    window.showButtonSpinner(btn);
    
    const formData = new FormData(this);
    
    // Fix is_active checkbox
    const isActiveCheckbox = document.querySelector('#addEducationLevelForm input[name="is_active"]');
    if (isActiveCheckbox) {
        formData.set('is_active', isActiveCheckbox.checked ? '1' : '0');
    }
    
    fetch('/admin/education-levels', {
        method: 'POST',
        headers: { 
    ...
    .then(data => {
        if (data.success) {
            window.showToast('success', data.message);
            const modal = bootstrap.Modal.getInstance(document.getElementById('kt_modal_add_education_level'));
            if (modal) modal.hide();
            this.reset();
            loadEducationLevels();
        } else {
            if (data.errors) {
                ...
                window.showToast('error', data.message || 'Failed to create education level');
            }
        }
    })
    .catch(err => {
        console.error('Error:', err);
        window.showToast('error', 'Failed to create education level: ' + err.message);
    })
    .finally(() => window.hideButtonSpinner(btn));
});

// Edit Education Level Form
document.getElementById('editEducationLevelForm')?.addEventListener('submit', function(e) {
    e.preventDefault();
    const btn = document.getElementById('editEducationLevelBtn');
    window.showButtonSpinner(btn);
    const id = document.getElementById('edit_education_level_id').value;
    
    const formData = new FormData(this);
    formData.append('_method', 'PUT');
    
    const isActiveCheckbox = document.querySelector('#editEducationLevelForm input[name="is_active"]');
    if (isActiveCheckbox) {
        formData.set('is_active', isActiveCheckbox.checked ? '1' : '0');
    }
    ...
    .then(data => {
        if (data.success) {
            window.showToast('success', data.message);
            const modal = bootstrap.Modal.getInstance(document.getElementById('kt_modal_edit_education_level'));
            if (modal) modal.hide();
            loadEducationLevels();
        } else {
    ...
    .catch(err => {
        console.error('Error:', err);
        window.showToast('error', 'Failed to update education level: ' + err.message);
    })
    .finally(() => window.hideButtonSpinner(btn));
});
