...
@section('title', 'Jobs by Source Report')
@section('page_title', 'Jobs by Source Report')
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item text-muted">By Source</li>
@endsection
...
<div class="card card-flush shadow-sm mb-5">
    <div class="card-body py-4">
        <form method="GET" action="{{ route('admin.jobs-reports.source') }}" class="row g-3 align-items-end">
            <div class="col-md-3">
            ...
            <div class="col-md-2">
                <label class="fw-semibold fs-7 mb-1">Source</label>
                <select name="job_source" class="form-select form-select-solid" data-control="select2">
                    <option value="">All Sources</option>
                    @foreach($jobSources as $source)
                        <option value="{{ $source }}" {{ ($jobSource ?? '') == $source ? 'selected' : '' }}>
                            @php
                                $sourceLabels = [
                            ...
                            @endphp
                            {{ $sourceLabels[$source] ?? ucfirst(str_replace('_', ' ', $source)) }}
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="card-body py-3 px-4">
                <div class="d-flex align-items-center">
                    <div class="symbol symbol-35px symbol-circle bg-light-primary me-2">
                        <i class="ki-duotone ki-link fs-2 text-primary">
                            <span class="path1"></span><span class="path2"></span>
                        </i>
                    </div>
                    <div class="flex-grow-1 min-w-0">
                        <span class="text-gray-600 fw-semibold d-block text-truncate fs-7">Total Sources</span>
                        <span class="fw-bold text-gray-800" style="font-size: clamp(0.8rem, 2vw, 1.5rem);">{{ $sourceBreakdown->total() }}</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card-body py-3 px-4">
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-grow-1 min-w-0">
                        ...
                        <span class="fw-bold text-gray-800" style="font-size: clamp(0.8rem, 2vw, 1.5rem);">{{ number_format($sourceBreakdown->sum('total_views')) }}</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card-body py-3 px-4">
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-grow-1 min-w-0">
                        ...
                        <span class="fw-bold text-gray-800" style="font-size: clamp(0.8rem, 2vw, 1.5rem);">{{ number_format($sourceBreakdown->sum('total_applications')) }}</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card-body py-3 px-4">
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-grow-1 min-w-0">
                        ...
                        <span class="fw-bold text-gray-800" style="font-size: clamp(0.8rem, 2vw, 1.5rem);">{{ $sourceBreakdown->sum('job_count') }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="col-12">
        <div class="card card-flush shadow-sm">
            <div class="card-header py-3">
                <h3 class="card-title fs-5 fw-bold">Source Breakdown</h3>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table align-middle table-row-dashed fs-6 gy-3">
                        ...
                        <tbody>
                            @php
                                $totalJobs = $sourceBreakdown->sum('job_count');
                            @endphp
                            @forelse($sourceBreakdown as $index => $item)
                                @php
                                    $sourceLabels = [
                                        'competitor_website' => 'Competitor Website',
                                        'whatsapp' => 'WhatsApp',
                                    ...
                                    $displayName = $sourceLabels[$item->job_source] ?? ucfirst(str_replace('_', ' ', $item->job_source));
                                    $percentage = $totalJobs > 0 ? ($item->job_count / $totalJobs) * 100 : 0;
                                @endphp
                                <tr>
                                    <td>{{ $sourceBreakdown->firstItem() + $index }}</td>
                                    <td>
                                        <span class="fw-bold">{{ $displayName }}</span>
                                        <div class="text-muted fs-8">{{ $item->job_source }}</div>
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="text-center text-muted py-5">No sources found</td>
                                </tr>
                            @endforelse
                        </tbody>
                        <tfoot>
                            <tr class="fw-bold">
                                ...
                                <td class="text-center">{{ $sourceBreakdown->sum('job_count') }}</td>
                                <td class="text-end">{{ number_format($sourceBreakdown->sum('total_views')) }}</td>
                                <td class="text-end">{{ number_format($sourceBreakdown->sum('total_applications')) }}</td>
                ...
                <div class="d-flex justify-content-between align-items-center mt-5">
                    ...
                    <div>
                        {{ $sourceBreakdown->appends(request()->except('page'))->links('pagination::bootstrap-5') }}
                    </div>
                </div>
            </div>
        </div>
...
<div class="row g-5 g-xl-10 mt-5">
    <div class="col-12">
        <div class="card card-flush shadow-sm">
            <div class="card-header py-3">
                <h3 class="card-title fs-5 fw-bold">Source Summary</h3>
            </div>
            <div class="card-body">
                            ...
                            <tbody>
                                @foreach($sourceBreakdown as $item)
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end">{{ number_format($item->total_views > 0 ? $item->total_views / $item->job_count : 0, 1) }}</td>
                                        <td class="text-end">{{ number_format($item->total_applications > 0 ? $item->total_applications / $item->job_count : 0, 1) }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
        ...
        <div class="card card-flush shadow-sm">
            <div class="card-body py-3">
                <div class="d-flex justify-content-end gap-2">
                    <a href="{{ route('admin.jobs-reports.export', ['type' => 'source']) . '?' . http_build_query(request()->except('page', 'per_page')) }}" class="btn btn-sm btn-success">
