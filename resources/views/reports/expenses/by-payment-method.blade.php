...
@section('title', 'Expenses by Payment Method')
@section('page_title', 'Expenses by Payment Method')
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item text-muted">By Payment Method</li>
@endsection
...
<div class="card card-flush shadow-sm mb-5">
    <div class="card-body py-4">
        <form method="GET" action="{{ route('admin.expense-reports.payment-method') }}" class="row g-3 align-items-end">
            <div class="col-md-3">
            ...
            <div class="col-md-3">
                <label class="fw-semibold fs-7 mb-1">Payment Method</label>
            ...
            <div class="card-body py-3 px-4">
                <div class="d-flex align-items-center">
                    <div class="symbol symbol-35px symbol-circle bg-light-primary me-2">
                        <i class="ki-duotone ki-card fs-2 text-primary">
                            <span class="path1"></span><span class="path2"></span>
                        </i>
                    </div>
                    <div class="flex-grow-1 min-w-0">
                        <span class="text-gray-600 fw-semibold d-block text-truncate fs-7">Total Methods</span>
                        <span class="fw-bold text-gray-800" style="font-size: clamp(0.8rem, 2vw, 1.5rem);">{{ $methodBreakdown->count() }}</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card-body py-3 px-4">
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-grow-1 min-w-0">
                        ...
                        <span class="fw-bold text-gray-800" style="font-size: clamp(0.8rem, 2vw, 1.5rem);">UGX {{ number_format($methodBreakdown->sum('total_amount') / 100, 0) }}</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card-body py-3 px-4">
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-grow-1 min-w-0">
                        ...
                        <span class="fw-bold text-gray-800" style="font-size: clamp(0.8rem, 2vw, 1.5rem);">{{ $methodBreakdown->sum('transaction_count') }}</span>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-grow-1 min-w-0">
                        ...
                        <span class="fw-bold text-gray-800" style="font-size: clamp(0.8rem, 2vw, 1.5rem);">
                            @php
                                $totalTrans = $methodBreakdown->sum('transaction_count');
                                $totalAmt = $methodBreakdown->sum('total_amount');
                                $avgTrans = $totalTrans > 0 ? $totalAmt / $totalTrans : 0;
                            @endphp
                            UGX {{ number_format($avgTrans / 100, 0) }}
                        </span>
                    </div>
                </div>
            </div>
...
<div class="card card-flush shadow-sm">
    <div class="card-header py-3">
        <h3 class="card-title fs-5 fw-bold">Payment Method Breakdown</h3>
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table align-middle table-row-dashed fs-6 gy-3">
                ...
                <tbody>
                    @php $grandTotal = $methodBreakdown->sum('total_amount'); @endphp
                    @forelse($methodBreakdown as $item)
                        <tr>
                            <td>
                                <span class="fw-bold">{{ $item->method_name }}</span>
                                @if($item->method_type)
                                    <span class="badge badge-light-primary ms-2">{{ ucfirst($item->method_type) }}</span>
                                @endif
                            </td>
                            <td>{{ ucfirst($item->method_type ?? 'N/A') }}</td>
                            ...
                            <td class="text-end">UGX {{ number_format($item->average_transaction / 100, 0) }}</td>
                            <td class="text-end">UGX {{ number_format($item->max_transaction / 100, 0) }}</td>
                            <td class="text-end">UGX {{ number_format($item->min_transaction / 100, 0) }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center text-muted py-5">No payment methods found</td>
                        </tr>
                    @endforelse
                </tbody>
                <tfoot>
                    <tr class="fw-bold">
                        <td>Total</td>
                        <td></td>
                        <td class="text-center">{{ $methodBreakdown->sum('transaction_count') }}</td>
        ...
        <div class="d-flex justify-content-between align-items-center mt-5">
            <div class="text-muted fs-7">
                Showing {{ $methodBreakdown->firstItem() ?? 0 }} to {{ $methodBreakdown->lastItem() ?? 0 }} of {{ $methodBreakdown->total() }} entries
            </div>
            <div>
                {{ $methodBreakdown->appends(request()->except('page'))->links() }}
            </div>
        </div>
    </div>
</div>

<!-- Payment Method Types Chart -->
<div class="card card-flush shadow-sm mt-5">
    <div class="card-header py-3">
        <h3 class="card-title fs-5 fw-bold">Summary by Payment Method Type</h3>
    </div>
    <div class="card-body">
        @php
            $typeSummary = $methodBreakdown->groupBy('method_type')->map(function($items) {
                return [
                    'type' => $items->first()->method_type ?? 'Other',
        ...
        @if($typeSummary->count() > 0)
            <div class="table-responsive">
                <table class="table align-middle table-row-dashed fs-6 gy-3">
                    ...
                    <tbody>
                        @foreach($typeSummary as $item)
                            <tr>
                                <td><span class="fw-bold">{{ ucfirst($item['type']) }}</span></td>
                                <td class="text-center">{{ $item['count'] }}</td>
                                <td class="text-end fw-bold text-success">UGX {{ number_format($item['total'] / 100, 0) }}</td>
                                <td class="text-center">
                                    <div class="d-flex align-items-center gap-2 justify-content-center">
                                        <span>{{ $grandTotal > 0 ? number_format(($item['total'] / $grandTotal) * 100, 1) : 0 }}%</span>
                                        <div class="progress w-50" style="height: 6px;">
                                            <div class="progress-bar bg-info" style="width: {{ $grandTotal > 0 ? ($item['total'] / $grandTotal) * 100 : 0 }}%;"></div>
                                        </div>
                                    </div>
                                </td>
                            </tr>
        ...
        @else
            <div class="text-center py-5 text-muted">No type data available</div>
        @endif
    </div>
</div>
...
<div class="card card-flush shadow-sm mt-5">
    <div class="card-header py-3">
        <h3 class="card-title fs-5 fw-bold">Top 5 Payment Methods</h3>
    </div>
    <div class="card-body">
        @php $topMethods = $methodBreakdown->take(5); @endphp
        @if($topMethods->count() > 0)
            <div class="table-responsive">
                <table class="table align-middle table-row-dashed fs-6 gy-3">
                    ...
                    <tbody>
                        @foreach($topMethods as $index => $item)
                            <tr>
                                <td><span class="badge badge-light-primary">{{ $index + 1 }}</span></td>
                                <td><span class="fw-bold">{{ $item->method_name }}</span></td>
                                <td class="text-end fw-bold text-success">UGX {{ number_format($item->total_amount / 100, 0) }}</td>
                                <td class="text-center">{{ $item->transaction_count }}</td>
                                <td class="text-center">
                                    <div class="d-flex align-items-center gap-2 justify-content-center">
        ...
        <div class="card card-flush shadow-sm">
            <div class="card-body py-3">
                <div class="d-flex justify-content-end gap-2">
                    <a href="{{ route('admin.expense-reports.export', ['type' => 'payment-method']) . '?' . http_build_query(request()->except('page', 'per_page')) }}" class="btn btn-sm btn-success">
