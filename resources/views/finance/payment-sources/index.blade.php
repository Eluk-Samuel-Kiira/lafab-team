...
@section('page_title', 'Payment Sources')
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item text-muted">Payment Sources</li>
@endsection
...
<div class="card card-flush">
    <div class="card-header mt-6">
        <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1 me-5">
                ...
                <input type="text" id="searchInput" class="form-control form-control-solid w-250px ps-13" placeholder="Search sources..." />
            </div>
        </div>
    ...
    <div class="card-body pt-0">
        <div id="loadingSpinner" class="text-center py-10 d-none">
            ...
            <p class="mt-3 text-muted">Loading sources...</p>
        </div>
        ...
        <div id="noDataMessage" class="text-center py-10 d-none">
            ...
            <p class="text-muted">No payment sources found.</p>
        </div>
    </div>
</div>

<!-- Add Source Modal -->
<div class="modal fade" id="kt_modal_add_source" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered mw-600px">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="fw-bold">Add Payment Source</h2>
            ...
            <div class="modal-body scroll-y mx-5 my-7">
                <form id="addSourceForm">
                    ...
                    <div class="fv-row mb-7">
                        ...
                        <input type="text" class="form-control form-control-solid" name="name" placeholder="e.g., Job Advertisers" required />
                    </div>
                    
                    <div class="fv-row mb-7">
                        <label class="required fw-semibold fs-6 mb-2">Slug</label>
                        <input type="text" class="form-control form-control-solid" name="slug" placeholder="e.g., job_advertisers" required />
                    ...
                    <div class="fv-row mb-7">
                        ...
                        <textarea class="form-control form-control-solid" name="description" rows="3" placeholder="Description of this payment source"></textarea>
                    </div>
                    ...
                    <div class="text-center pt-15">
                        ...
                        <button type="submit" class="btn btn-primary" id="addSourceBtn">
                            <span class="indicator-label">Create Source</span>
...
<!-- Edit Source Modal -->
<div class="modal fade" id="kt_modal_edit_source" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered mw-600px">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="fw-bold">Edit Payment Source</h2>
                <div class="btn btn-icon btn-sm btn-active-icon-primary" data-bs-dismiss="modal">
                    <i class="ki-duotone ki-cross fs-1"><span class="path1"></span><span class="path2"></span></i>
                </div>
            </div>
            <div class="modal-body scroll-y mx-5 my-7">
                <form id="editSourceForm">
                    ...
                    <div class="text-center pt-15">
                        ...
                        <button type="submit" class="btn btn-primary" id="editSourceBtn">
                            <span class="indicator-label">Update Source</span>
                            <span class="indicator-progress">Please wait... <span class="spinner-border spinner-border-sm align-middle ms-2"></span></span>
                        </button>
                    </div>
                </form>
...
function loadSources() {
    ...
    let url = `/admin/payment-sources/data?page=${currentPage}&per_page=20`;
    ...
    fetch(url)
        ...
        .catch(err => {
            spinner.classList.add('d-none');
            window.showToast('error', 'Failed to load sources');
        });
}
...
function renderSourcesTable(sources) {
    ...
    sources.forEach(source => {
        ...
        row.insertCell(0).innerHTML = `<span class="fw-bold">${source.id}</span>`;
        row.insertCell(1).innerHTML = `<div class="fw-bold">${escapeHtml(source.name)}</div><div class="text-muted fs-7">${escapeHtml(source.slug)}</div>`;
        row.insertCell(2).innerHTML = `<span class="text-muted fs-7">${source.slug}</span>`;
        row.insertCell(3).innerHTML = `<i class="ki-duotone ${source.icon || 'ki-category'} fs-2x text-${source.color || 'primary'}"><span class="path1"></span><span class="path2"></span></i>`;
        row.insertCell(4).innerHTML = `<span class="badge badge-light-${source.color || 'secondary'}">${source.color || 'default'}</span>`;
        row.insertCell(5).innerHTML = `<span class="badge badge-light-${source.category === 'revenue' ? 'success' : (source.category === 'capital' ? 'info' : 'secondary')}">${source.category}</span>`;
        row.insertCell(6).innerHTML = source.is_active ? '<span class="badge badge-light-success">Active</span>' : '<span class="badge badge-light-danger">Inactive</span>';
        row.insertCell(7).innerHTML = `<span class="fw-bold">${source.deposits_count}</span>`;
        row.insertCell(8).innerHTML = source.sort_order;
        row.insertCell(9).innerHTML = `
            <div class="d-flex justify-content-end gap-2">
                <button class="btn btn-sm btn-icon btn-light" onclick="editSource(${source.id})" title="Edit">
                ...
                <button class="btn btn-sm btn-icon btn-light" onclick="deleteSource(${source.id}, '${escapeHtml(source.name)}', ${source.deposits_count})" title="Delete">
...
window.changePage = function(page) {
    if (page !== currentPage && page > 0) { currentPage = page; loadSources(); }
};

window.editSource = function(id) {
    fetch(`/admin/payment-sources/${id}`)
        .then(res => res.json())
        .then(data => {
            document.getElementById('edit_source_id').value = data.id;
            ...
            new bootstrap.Modal(document.getElementById('kt_modal_edit_source')).show();
        });
};

window.deleteSource = function(id, name, depositsCount) {
    let message = `Are you sure you want to delete source "${name}"?`;
    if (depositsCount > 0) {
    ...
    if (confirm(message)) {
        fetch(`/admin/payment-sources/${id}`, {
            ...
            if (data.success) { window.showToast('success', data.message); loadSources(); }
...
document.getElementById('addSourceForm')?.addEventListener('submit', function(e) {
    ...
    fetch('{{ route("admin.payment-sources.store") }}', {
        method: 'POST',
        ...
        if (data.success) {
            ...
            bootstrap.Modal.getInstance(document.getElementById('kt_modal_add_source'))?.hide();
            this.reset();
            loadSources();
        } else {
            window.showToast('error', data.message);
        }
    }).catch(err => window.showToast('error', 'Failed to create source')).finally(() => window.hideButtonSpinner(btn));
});
...
document.getElementById('editSourceForm')?.addEventListener('submit', function(e) {
    ...
    window.showButtonSpinner(btn);
    const id = document.getElementById('edit_source_id').value;
    ...
    fetch(`/admin/payment-sources/${id}`, {
        ...
        if (data.success) {
            window.showToast('success', data.message);
            bootstrap.Modal.getInstance(document.getElementById('kt_modal_edit_source'))?.hide();
            loadSources();
        } else {
            window.showToast('error', data.message);
        }
    }).catch(err => window.showToast('error', 'Failed to update source')).finally(() => window.hideButtonSpinner(btn));
});
